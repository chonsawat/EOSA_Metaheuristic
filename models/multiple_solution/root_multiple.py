...
class RootAlgo(object):
    ...
    '''
    def _create_infected_solution__(self, minmax=0, infected_size=None):
        solution = np.random.uniform(self.domain_range[0], self.domain_range[1], infected_size)
        fitness = self._fitness_infected_model__(solution=solution, minmax=minmax, infected_size=infected_size)
        return [solution, fitness]
    
    def _fitness_infected_model__(self, solution=None, minmax=0, infected_size=None):
        return self.objective_func(solution, infected_size) if minmax == 0 \
            else 1.0 / self.objective_func(solution, infected_size)
    '''
