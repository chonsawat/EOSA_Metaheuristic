...
from models.multiple_solution.root import Root
...
class BaseMVO(Root):
    ...
    def __init__(self, obj_func=None, lb=None, ub=None, problem_size=50, batch_size=10, verbose=True,
        ...
        self.a_scatter=None
        self.spop= [self._create_solution__(minmax=1) for _ in range(self.pop_size)]
        #self._show_all_fits()
    ...
    def train(self):
        ...
        for epoch in range(0, self.epoch):
            ...
            for i in range(1, self.pop_size):  # Starting from 1 since 0 is the elite
                ...
                if (fit < pop[i][self.ID_FIT]).numpy().all():
                    pop[i] = [black_hole_pos, fit]

                if i % self.batch_size:
                    g_best = self.update_global_best_solution(pop, self.ID_MIN_PROB, g_best)
            self.loss_train.append(g_best[self.ID_FIT])
            epoch_time = round(time() - epoch_time, 4)
            self.epoch_times.append(epoch_time)
            if self.verbose:
                print("> Epoch: {}, Epoch time {}, Best fit: {}".format(epoch + 1, epoch_time, g_best[self.ID_FIT]))
